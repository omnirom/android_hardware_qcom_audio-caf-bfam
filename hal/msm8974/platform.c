...
void *platform_init(struct audio_device *adev)
{
    ...
    char d_mixer_paths_final[PROPERTY_VALUE_MAX + sizeof(MIXER_XML_PATH_PREFIX)];
    char d_mixer_paths_aux_final[PROPERTY_VALUE_MAX + sizeof(MIXER_XML_PATH_PREFIX)];
#endif
    ...
    while (snd_card_num < MAX_SND_CARD) {
        ...
        if (!my_data->hw_info) {
            ...
            if(strlen(d_mixer_paths_xml) > PROPERTY_VALUE_MAX) {
                strcpy(d_mixer_paths_xml,MIXER_PATH_DEF);
                ALOGE("Property audio.mixer_paths.config is bigger then %i, using default value.", PROPERTY_VALUE_MAX);
            }
            ...
            if(strlen(d_mixer_paths_aux_xml) > PROPERTY_VALUE_MAX) {
                strcpy(d_mixer_paths_aux_xml,MIXER_PATH_AUX_DEF);
                ALOGE("Property audio.mixer_paths_aux.config is bigger then %i, using default value.", PROPERTY_VALUE_MAX);
            }
