...
void *platform_init(struct audio_device *adev)
{
    ...
    while (snd_card_num < MAX_SND_CARD) {
        ...
        if (!my_data->hw_info) {
            ...
            rc = property_get("audio.mixer_paths.config", d_mixer_paths_xml + (sizeof(MIXER_XML_PATH_PREFIX) - 1), MIXER_PATH_DEF);
            if (!rc)
            ...
            rc = property_get("audio.mixer_paths_aux.config", d_mixer_paths_aux_xml + (sizeof(MIXER_XML_PATH_PREFIX) - 1), MIXER_PATH_AUX_DEF);
            if (!rc)
